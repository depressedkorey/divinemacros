...
class CommunityProfile(db.Model):
    ...
    display_name = db.Column(db.String(32), unique=True, nullable=False, index=True)
    public_discord = db.Column(db.Boolean, nullable=False, default=True)
    updated_at = db.Column(
        db.DateTime, nullable=False,
        default=lambda: datetime.now(timezone.utc),
        onupdate=lambda: datetime.now(timezone.utc),
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'discordUsername': self.discord_username or '',
            'avatarUrl': self.avatar_url or '',
            'displayName': self.display_name,
            'publicDiscord': bool(self.public_discord),
...
class CommunityChatMessage(db.Model):
    ...
    id = db.Column(db.String(32), primary_key=True, default=lambda: secrets.token_hex(8))
