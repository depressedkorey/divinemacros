...
def _license_plan(item: License) -> str:
    ...
    if tier in {'lifetime', 'lifetime_pro', 'permanent', 'forever'}:
        return 'lifetime_pro'
    if tier == 'lifetime_core':
        return 'lifetime_core'
    if tier == '3month':
        return '3month'
    return 'monthly'
...
def _normalize_checkout_plan(raw_plan: str) -> str:
    ...
    if text in {'lifetime', 'lifetime_pro'}:
        return 'lifetime_pro'
    if text == 'lifetime_core':
        return 'lifetime_core'
    if text == '3month':
        return '3month'
    return 'monthly'
...
def _plan_display_name(plan: str) -> str:
    ...
    if normalized == '3month':
        return 'Pro'
    if normalized == 'lifetime_core':
        return 'Lifetime Core'
    if normalized == 'lifetime_pro':
        return 'Lifetime Pro'
    return 'Core'
...
def _pick_release_asset(
    ...
    *,
    ...
    for suffix in ('.exe', '.zip', '.msi'):
        for asset in assets:
            name = str(asset.get('name') or '').strip().lower()
            # Skip setup/installer exes â€” only use them as last resort
...
def _secure_install_allowed(license_row: License | None) -> bool:
    if license_row is None or license_row.is_revoked or _is_expired(license_row):
        return False
    return _license_plan(license_row) in {'3month', 'lifetime_pro'}
...
def community_profile_proxy():
    ...
    with _state_lock:
        ...
        users = store.get('users') if isinstance(store.get('users'), dict) else {}
        current = dict(users.get(discord_id) or _community_default_profile(actor))
        current['discordId'] = discord_id
        current['discordUsername'] = str(actor.get('discordUsername') or current.get('discordUsername') or '').strip()
        current['avatarUrl'] = str(actor.get('avatarUrl') or current.get('avatarUrl') or '').strip()

        if request.method == 'POST':
            payload = request.get_json(silent=True) or {}
            display_name = _community_display_name(payload.get('displayName') or current.get('displayName') or current.get('discordUsername') or 'Divine User')
            if _community_profile_name_taken(store, display_name, discord_id):
                return jsonify({'ok': False, 'error': 'That marketplace name is already taken.'}), 409
            current['displayName'] = display_name
            current['publicDiscord'] = bool(payload.get('publicDiscord', True))
...
def community_submit_proxy():
    ...
    with _state_lock:
        ...
        if actor and actor.get('discordId'):
            discord_id = str(actor.get('discordId') or '').strip()
            users = store.get('users') if isinstance(store.get('users'), dict) else {}
            profile = dict(users.get(discord_id) or _community_default_profile(actor))
            ...
            entry['author'] = public_profile.get('discordUsername') if public_profile.get('publicDiscord') and public_profile.get('discordUsername') else public_profile.get('displayName', 'Divine User')
        entries = list(store.get('entries') or [])
...
def community_chat():
    ...
    if request.method == 'POST':
        ...
        message = {
            ...
            'discordId': str(actor.get('discordId') or ''),
            'author': _community_display_name(actor.get('discordUsername') or 'Divine User'),
            'avatarUrl': str(actor.get('avatarUrl') or ''),
    ...
    with _state_lock:
        chat = list(_community_read_store().get('chat') or [])
    chat_discord_ids = {
    ...
    return jsonify({'ok': True, 'messages': chat[-limit:]}), 200
...
def dashboard_me():
    ...
    if active is not None and not active.is_revoked and not _is_expired(active):
        ...
        summary = {
            ...
            'nextBillingDate': _license_expires_iso(active) if plan in {'monthly', '3month'} else None,
...
def create_checkout():
    ...
    plan = _normalize_checkout_plan(request.args.get('plan') or 'monthly')
...
def pricing_info():
    ...
    monthly_value = float(current_app.config.get('MONTHLY_PRICE_DISPLAY', 5) or 5)
    three_month_value = float(current_app.config.get('PRICE_3MONTH_DISPLAY', 10) or 10)
    lifetime_core_value = float(current_app.config.get('LIFETIME_CORE_PRICE_DISPLAY', 12.99) or 12.99)
    lifetime_pro_value = float(current_app.config.get('LIFETIME_PRO_PRICE_DISPLAY', 19.99) or 19.99)
    plans = {
        'monthly': {
            'name': 'Core',
            'desc': 'Full Divine Macros access with monthly billing.',
        ...
        '3month': {
            'name': 'Pro',
            'desc': 'Full access for 3 months â€” one payment.',
            'price': f'${three_month_value:.2f}'.rstrip('0').rstrip('.'),
            'amount': int(round(three_month_value * 100)),
            'period': '/ 3 months',
        },
        'lifetime_core': {
            'name': 'Lifetime Core',
    ...
    plans['lifetime'] = dict(plans['lifetime_pro'])
...
def checkout_session():
    ...
    data = request.get_json(silent=True) or {}
    plan = _normalize_checkout_plan(data.get('plan') or 'monthly')
...
def create_payment_intent():
    ...
    if plan not in {'lifetime_core', 'lifetime_pro'}:
    ...
    try:
        import stripe as _stripe
        _stripe.api_key = stripe_key
        amount = 1299 if plan == 'lifetime_core' else 1999
...
def dashboard_installer_command():
    ...
    rows = _licenses_for_user(user)
    active = _choose_summary_license(rows)
    if not active or active.tier == 'standalone':
        return jsonify({
            'ok': False,
    ...
    if not _secure_install_allowed(active):
        return jsonify({
            'ok': False,
            'error': 'pro_required',
            'message': 'Secure install is available for Pro and Lifetime Pro plans only.',
        }), 403
...
def _send_referrer_dm(affiliate_code: str, buyer_discord_id: str, plan: str) -> None:
    ...
    if normalized_plan == 'lifetime_core':
        plan_label = 'Lifetime Core license'
    elif normalized_plan == 'lifetime_pro':
        plan_label = 'Lifetime Pro license'
    elif normalized_plan == '3month':
...
def _send_purchase_dm(discord_id: str, plan: str, product_name: str | None = None) -> None:
    ...
    elif _normalize_checkout_plan(plan) == 'lifetime_core':
        purchase_line = 'your **Lifetime Core** Divine Macros license'
    elif _normalize_checkout_plan(plan) == 'lifetime_pro':
        purchase_line = 'your **Lifetime Pro** Divine Macros license'
    elif _normalize_checkout_plan(plan) == '3month':
        purchase_line = 'your **3-Month** Divine Macros subscription'
    else:
        purchase_line = 'your **Monthly** Divine Macros subscription'
...
def _post_sale_notification(discord_id: str, plan: str, charged_cents: int,
    ...
    if is_standalone:
        ...
        normalized_plan = _normalize_checkout_plan(plan)
        if normalized_plan == 'lifetime_core':
            plan_label = 'Lifetime Core'
            plan_color = 0x10bb82
        elif normalized_plan == 'lifetime_pro':
            plan_label = 'Lifetime Pro'
            plan_color = 0x7c3aed
        elif normalized_plan == '3month':
            plan_label = 'Pro'
            plan_color = 0xffd60a
