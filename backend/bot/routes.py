...
from models import ClientFeatureLock, ClientNotification, License, Product, UserEntitlement
...
def _meta_access_role(meta: dict) -> str:
...
def bot_key_create():
    ...
    access = str(data.get('access') or data.get('tier') or 'core_monthly').strip().lower()
    access = {
        'monthly': 'core_monthly', 'core': 'core_monthly',
        'pro': 'pro_monthly', '3month': 'pro_monthly',
        'lifetime': 'pro_lifetime', 'lifetime_pro': 'pro_lifetime',
        'lifetime_core': 'core_lifetime',
    }.get(access, access)
    if access not in {'core_monthly', 'core_lifetime', 'pro_monthly', 'pro_lifetime'}:
        return jsonify({'ok': False, 'error': 'access must be core_monthly, core_lifetime, pro_monthly, or pro_lifetime'}), 400
    access_role = str(data.get('access_role') or data.get('role') or 'customer').strip().lower()
    if access_role not in {'customer', 'moderator', 'admin'}:
        return jsonify({'ok': False, 'error': 'role must be customer, moderator, or admin'}), 400

    expires_at = None
    if access in {'core_monthly', 'pro_monthly'}:
    ...
    if access in {'core_monthly', 'pro_monthly'} and discord_id_val:
...
def bot_key_update():
    ...
    if 'access' in data or 'tier' in data:
        ...
        if access not in {'core_monthly', 'core_lifetime', 'pro_monthly', 'pro_lifetime'}:
            return jsonify({'ok': False, 'error': 'access must be core_monthly, core_lifetime, pro_monthly, or pro_lifetime'}), 400
        item.tier = access
        if access in {'pro_lifetime', 'core_lifetime'}:
    ...
    if 'days' in data and data.get('days') is not None:
        ...
        if item.tier not in {'core_lifetime', 'pro_lifetime', 'lifetime_core', 'lifetime_pro', 'lifetime'}:
            item.tier = 'core_monthly'
    ...
    if 'access_role' in data or 'role' in data:
        role = str((data.get('access_role') if 'access_role' in data else data.get('role')) or '').strip().lower()
        if role and role not in {'customer', 'moderator', 'admin'}:
            return jsonify({'ok': False, 'error': 'role must be customer, moderator, or admin'}), 400
        if role:
            meta['access_role'] = role
    item.extra_metadata = meta
...
def bot_key_extend():
    ...
    if str(item.tier or '').lower() not in {'core_lifetime', 'pro_lifetime', 'lifetime_core', 'lifetime_pro', 'lifetime'}:
        if str(item.tier or '').lower() not in {'core_monthly', 'pro_monthly'}:
            item.tier = 'core_monthly'
    item.session_nonce = secrets.token_hex(16)
    db.session.commit()
...
def bot_boost_extend():
    """Manage a dedicated two-boost Core reward license."""
    ip = _client_ip()
    ...
    if action not in {'claim', 'deactivate'}:
        return jsonify({'ok': False, 'error': 'action must be claim or deactivate'}), 400
    ...
    item = next(
        (lic for lic in licenses if bool((lic.extra_metadata or {}).get('boost_reward'))),
        None,
    )
    active_regular = next(
        (
            lic for lic in licenses
            if not bool((lic.extra_metadata or {}).get('boost_reward')) and lic.is_active()
        ),
        None,
    )
    ...
    if action == 'deactivate':
        if item is None:
            return jsonify({'ok': True, 'reason': 'not_active', 'boost_core_active': False}), 200

        meta = dict(item.extra_metadata or {})
        meta['boost_core_active'] = False
        meta['boost_claims'] = 0
        meta['boost_deactivated_at'] = datetime.now(timezone.utc).isoformat()
        item.is_revoked = True
        item.expires_at = None
        item.extra_metadata = meta
        item.session_nonce = secrets.token_hex(16)
        db.session.commit()
        audit_event('bot.boost_core.deactivated', ip=ip, license_id=item.id, discord_id=discord_id)
        return jsonify({
            'ok': True,
            'action': 'deactivated',
            'boost_core_active': False,
            'boost_count': 0,
        }), 200

    if active_regular is not None:
        return jsonify({
            'ok': False,
            'reason': 'existing_active',
            'error': 'An active license is already linked to this Discord account. It was not changed.',
        }), 200

    if item is None:
        normalized = _generate_key()
        while License.query.filter_by(key=normalized).first() is not None:
            normalized = _generate_key()
        ...
        item.extra_metadata = {
            ...
            'boost_claims': 0,
            'boost_core_active': False,
            'boost_created_at': datetime.now(timezone.utc).isoformat(),
        }
        db.session.add(item)
        db.session.flush()

    meta = dict(item.extra_metadata or {})
    current_count = max(0, min(2, int(meta.get('boost_claims', meta.get('boost_extensions', 0)) or 0)))
    if bool(meta.get('boost_core_active')) or current_count >= 2:
        return jsonify({
            'ok': False,
            'reason': 'max_reached',
            'error': 'The two-boost Core reward is already active.',
            'boost_count': 2,
            'boost_core_active': True,
            'expiresAt': _to_iso(item.expires_at),
            'key': _format_key(item.key),
        }), 200

    try:
        verified_count = max(1, min(2, int(data.get('verified_boost_count') or 1)))
    except (TypeError, ValueError):
        verified_count = 1
    new_count = max(current_count, verified_count)
    if new_count == current_count:
        return jsonify({
            'ok': True,
            'action': 'already_verified',
            'boost_count': current_count,
            'boost_core_active': bool(meta.get('boost_core_active')),
            'expiresAt': _to_iso(item.expires_at),
            'key': _format_key(item.key) if current_count >= 2 else None,
            'license': _license_payload(item),
        }), 200

    meta['boost_claims'] = new_count
    meta['boost_last_claimed_at'] = datetime.now(timezone.utc).isoformat()

    if new_count == 2:
        meta['boost_core_active'] = True
        meta['boost_core_started_at'] = datetime.now(timezone.utc).isoformat()
        item.tier = 'core_monthly'
        item.expires_at = None
    ...
    audit_event(
        'bot.boost_core.claimed',
        ip=ip,
        license_id=item.id,
        discord_id=discord_id,
        boost_count=new_count,
        core_active=new_count == 2,
    )
    return jsonify({
        'ok': True,
        'action': 'core_activated' if new_count == 2 else 'first_boost_recorded',
        'boost_count': new_count,
        'boost_core_active': new_count == 2,
        'expiresAt': _to_iso(item.expires_at),
        'key': _format_key(item.key) if new_count == 2 else None,
        'license': _license_payload(item),
    }), 200
...
def bot_discord_grant():
    ...
    plan = str(data.get('plan', 'core_monthly')).strip().lower()
    plan = {
        'monthly': 'core_monthly', 'core': 'core_monthly',
        'pro': 'pro_monthly', '3month': 'pro_monthly',
        'lifetime': 'pro_lifetime', 'lifetime_pro': 'pro_lifetime',
    ...
    if not discord_id.isdigit():
        return jsonify({'ok': False, 'error': 'Invalid discord_id'}), 400
    if plan not in {'core_monthly', 'core_lifetime', 'pro_monthly', 'pro_lifetime'}:
        return jsonify({'ok': False, 'error': 'plan must be core_monthly, core_lifetime, pro_monthly, or pro_lifetime'}), 400
    if access_role not in {'customer', 'moderator', 'admin'}:
        return jsonify({'ok': False, 'error': 'role must be customer, moderator, or admin'}), 400
    expires_at = None
    ...
    while License.query.filter_by(key_hash=sha256_hex(normalized)).first() is not None:
        normalized = _generate_key()
    meta = {'discord_id': discord_id, 'access_role': access_role}
    _meta_set_text(meta, 'notes', data.get('note') or data.get('notes') or '')
    _meta_set_text(meta, 'email', data.get('email') or '')
    item = License(
...
def bot_discord_upgrade():
    ...
    for row in rows:
        ...
        if row.is_revoked:
            continue
        current_plan = str(row.tier or '').strip().lower()
        row.tier = 'pro_lifetime' if current_plan in {'pro', '3month', 'pro_monthly'} else 'core_lifetime'
        row.expires_at = None
        row.session_nonce = secrets.token_hex(16)
...
def claim_discord_role():
    ...
    return jsonify({'ok': True, 'tier': item.tier or 'core_monthly'}), 200
...
@bot_bp.route('/api/bot/client-notifications', methods=['POST'])
def bot_client_notification_create():
    ip = _client_ip()
    if not _check_rate('client_notification_create', 40):
        return jsonify(_RATE_ERR), 429
    if not _authorized():
        return jsonify(_AUTH_ERR), 401
    data, err = _require_json()
    ...
    audience = str(data.get('audience') or 'all').strip().lower()
    discord_id = str(data.get('discord_id') or '').strip()
    kind = str(data.get('kind') or 'info').strip().lower()
    title = str(data.get('title') or '').strip()[:120]
    message = str(data.get('message') or '').strip()[:3000]
    action_label = str(data.get('action_label') or '').strip()[:40]
    action_url = str(data.get('action_url') or '').strip()[:500]
    created_by = str(data.get('created_by') or 'discord-bot').strip()[:64]

    if audience not in {'all', 'discord'}:
        return jsonify({'ok': False, 'error': 'audience must be all or discord'}), 400
    if audience == 'discord' and not discord_id.isdigit():
        return jsonify({'ok': False, 'error': 'A Discord user is required for targeted notifications'}), 400
    if kind not in {'info', 'success', 'warning', 'danger', 'update'}:
        return jsonify({'ok': False, 'error': 'Invalid notification kind'}), 400
    if not title or not message:
        return jsonify({'ok': False, 'error': 'title and message are required'}), 400
    if action_url and not action_url.lower().startswith(('https://', 'http://')):
        return jsonify({'ok': False, 'error': 'action_url must be an http or https URL'}), 400

    try:
        expires_hours = int(data.get('expires_hours') or 168)
    except (TypeError, ValueError):
        expires_hours = 168
    ...
    row = ClientNotification(
        audience=audience,
        discord_id=discord_id if audience == 'discord' else None,
        kind=kind,
        title=title,
        message=message,
    ...
    db.session.commit()
    audit_event(
        'bot.client_notification.created',
        ip=ip,
    ...
    return jsonify({'ok': True, 'notification': row.to_dict()}), 201


_CLIENT_LOCK_CATEGORIES = {
    'create', 'library', 'community', 'account-menu',
    'crystal', 'sword', 'mace', 'cart', 'uhc', 'recorder', 'profiles',
    'messages', 'bindings', 'chat', 'account', 'my-macros', 'composer',
    'marketplace', 'themes', 'settings', 'changelog', 'admin',
}

_CLIENT_LOCK_MODULES = {
    'sa', 'sfa', 'aa', 'da', 'ap', 'hc', 'ac', 'kp', 'idh', 'oht',
    'fxp', 'pds', 'wt', 'sr', 'asb', 'sw', 'ls', 'kbd', 'tb', 'es',
    'pc', 'ss', 'ass', 'bs', 'ic', 'xb', 'dr', 'lw', 'la',
}


@bot_bp.route('/api/bot/client-locks', methods=['GET', 'POST'])
def bot_client_feature_locks():
    ip = _client_ip()
    if not _check_rate('client_feature_locks', 100):
        return jsonify(_RATE_ERR), 429
    if not _authorized():
        return jsonify(_AUTH_ERR), 401

    if request.method == 'GET':
        rows = ClientFeatureLock.query.order_by(ClientFeatureLock.category.asc()).all()
    ...
    updated_by = str(data.get('updated_by') or 'discord-bot').strip()[:64]
    valid = category in _CLIENT_LOCK_CATEGORIES or (
        category.startswith('module:') and category.removeprefix('module:') in _CLIENT_LOCK_MODULES
    )
    if not valid:
        return jsonify({'ok': False, 'error': 'Invalid client category or macro module'}), 400

    row = ClientFeatureLock.query.get(category)
    if row is None:
        row = ClientFeatureLock(category=category)
        db.session.add(row)
    ...
    db.session.commit()
    audit_event(
        'bot.client_lock.updated',
        ip=ip,
        category=category,
        locked=locked,
        updated_by=updated_by,
    )
    return jsonify({'ok': True, 'lock': row.to_dict()}), 200
