...
def bot_key_create():
    ...
    if access == 'lifetime':
        access = 'lifetime_pro'
    if access not in {'monthly', 'pro', '3month', 'lifetime_pro', 'lifetime_core'}:
        return jsonify({'ok': False, 'error': 'access must be monthly, pro, lifetime_pro, or lifetime_core'}), 400
    ...
    expires_at = None
    if access in {'monthly', 'pro', '3month'}:
        expires_at = _parse_iso_datetime(data.get('expiresAt') or data.get('expires_at'))
        if expires_at is None:
            default_days = 90 if access == '3month' else 30
            days = _parse_days(data.get('days'), default_days) or default_days
    ...
    if access in {'monthly', '3month'} and discord_id_val:
...
def bot_key_update():
    ...
    if 'access' in data or 'tier' in data:
        ...
        if access not in {'monthly', 'pro', '3month', 'lifetime_pro', 'lifetime_core'}:
            return jsonify({'ok': False, 'error': 'access must be monthly, pro, lifetime_pro, or lifetime_core'}), 400
        item.tier = access
        if access in {'lifetime_pro', 'lifetime_core'}:
...
def bot_discord_grant():
    ...
    if not discord_id.isdigit():
        return jsonify({'ok': False, 'error': 'Invalid discord_id'}), 400
    if plan not in {'monthly', 'pro', '3month', 'lifetime_pro', 'lifetime_core'}:
        return jsonify({'ok': False, 'error': 'plan must be monthly, pro, lifetime_pro, or lifetime_core'}), 400
    if access_role not in {'customer', 'moderator', 'admin'}:
        return jsonify({'ok': False, 'error': 'role must be customer, moderator, or admin'}), 400
    expires_at = None
    if plan in {'monthly', 'pro', '3month'}:
        expires_at = _parse_iso_datetime(data.get('expiresAt') or data.get('expires_at'))
        if expires_at is None:
            default_days = 90 if plan == '3month' else 30
            days = _parse_days(data.get('days'), default_days) or default_days
            expires_at = datetime.now(timezone.utc) + timedelta(days=days)
    normalized = _generate_key()
    while License.query.filter_by(key_hash=sha256_hex(normalized)).first() is not None:
...
def bot_client_feature_locks():
    ...
    if category not in _CLIENT_LOCK_CATEGORIES:
        return jsonify({'ok': False, 'error': 'Invalid client category'}), 400
