...
_PRODUCT_DOWNLOAD_REFS = {
    'zenith-single-anchor': 'ZenithSingleAnchor.exe',
    'zenith-safe-anchor':   'ZenithSafeAnchor.exe',
    'zenith-shield-break':  'ZenithShieldBreak.exe',
    'zenith-triggerbot':    'ZenithTriggerbot.exe',
    'zenith-stun-slam':     'ZenithStunSlam.exe',
    'zenith-pearl-catch':   'ZenithPearlCatch.exe',
    'zenith-breach-swap':   'ZenithBreachSwap.exe',
}
...
def create_app():
    ...
    for origin in app.config.get('WEB_CORS_ORIGINS', []) or []:
        text = str(origin or '').strip().rstrip('/')
        if text:
            _allowed_origins.add(text)
    site_origin = str(app.config.get('SITE_URL', '')).strip().rstrip('/')
    if site_origin:
        _allowed_origins.add(site_origin)
    frontend_origin = str(app.config.get('WEB_FRONTEND_ORIGIN', '')).strip().rstrip('/')
    if frontend_origin:
        _allowed_origins.add(frontend_origin)

    # Allow Tauri app origins for desktop client
    _allowed_origins.add('tauri://localhost')
    _allowed_origins.add('https://tauri.localhost')
    _allowed_origins.add('http://tauri.localhost')
    ...
    def apply_security_headers(response):
        ...
        if origin in _allowed_origins:
            ...
            response.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-License-Key, X-Auth-License'
