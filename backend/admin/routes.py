...
def admin_list_licenses():
    ...
    if tier in {'monthly', '3month', 'lifetime'}:
...
def admin_create_license():
    ...
    if tier not in {'monthly', '3month', 'lifetime'}:
        audit_event('admin.create.bad_request', ip=_client_ip(), reason='bad_tier')
        return jsonify({'ok': False, 'error': 'tier must be monthly, 3month, or lifetime'}), 400

    expires_at = None
    if tier in {'monthly', '3month'}:
        default_days = 90 if tier == '3month' else 30
        days = _parse_int(data.get('days', default_days), default_days, 1, 3650)
