...
def _env_float(name: str, default: float, minimum: float, maximum: float) -> float:
    """Parse a decimal env var safely and clamp to a bounded range."""
    raw = os.environ.get(name)
    if raw is None or str(raw).strip() == '':
        return default
    try:
...
class Config:
    ...
    SECRET_KEY = _env_first('ZENITH_SECRET_KEY', 'ZENITH_SECRET_KEY', 'DASHBOARD_SESSION_SECRET', 'ADMIN_SECRET')
    BOT_API_TOKEN = _env_first('BOT_API_SECRET', 'ZENITH_BOT_API_TOKEN', 'ZENITH_BOT_API_TOKEN')
    STORE_API_TOKEN = _env_first('STORE_API_TOKEN', 'ZENITH_STORE_API_TOKEN', 'ZENITH_STORE_API_TOKEN', 'BOT_API_SECRET')
    # [SECURITY HARDENING] Legacy bot bridge secret for /api/bot/* compatibility.
    BOT_API_SECRET = _env_first('ZENITH_BOT_API_SECRET', 'ZENITH_BOT_API_SECRET', 'BOT_API_SECRET', default=BOT_API_TOKEN)
    ...
    if _fly_db_url:
        ...
        DB_PATH = _env_first('ZENITH_DB_PATH', 'ZENITH_DB_PATH', default=_default_db_path)
    ...
    MAX_JSON_BODY_BYTES = _env_int('ZENITH_MAX_JSON_BODY_BYTES', _env_int('ZENITH_MAX_JSON_BODY_BYTES', 16_384, 2_048, 262_144), 2_048, 262_144)
    # [SECURITY HARDENING] Independent per-IP caps for admin/store API usage.
    ADMIN_RPM = _env_int('ZENITH_ADMIN_RPM', _env_int('ZENITH_ADMIN_RPM', 120, 10, 2_000), 10, 2_000)
    STORE_FULFILL_RPM = _env_int('ZENITH_STORE_FULFILL_RPM', _env_int('ZENITH_STORE_FULFILL_RPM', 60, 5, 1_000), 5, 1_000)
    # [SECURITY HARDENING] Optional request-signature replay window (seconds).
    REQUEST_SIG_SKEW_SECONDS = _env_int('ZENITH_REQUEST_SIG_SKEW_SECONDS', _env_int('ZENITH_REQUEST_SIG_SKEW_SECONDS', 300, 30, 3_600), 30, 3_600)

    # Website + dashboard compatibility
    SITE_URL = _env_first('SITE_URL', default='https://divinemacros.store')
    WEB_FRONTEND_ORIGIN = _env_first('WEB_FRONTEND_ORIGIN', default=SITE_URL).rstrip('/')
    WEB_CORS_ORIGINS = _env_csv('WEB_CORS_ORIGINS', default=WEB_FRONTEND_ORIGIN)
    DISCORD_OAUTH_CLIENT_ID = _env_first('ZENITH_DISCORD_OAUTH_CLIENT_ID', 'ZENITH_DISCORD_OAUTH_CLIENT_ID', 'DISCORD_OAUTH_CLIENT_ID', 'DISCORD_CLIENT_ID')
    DISCORD_OAUTH_CLIENT_SECRET = _env_first('ZENITH_DISCORD_OAUTH_CLIENT_SECRET', 'ZENITH_DISCORD_OAUTH_CLIENT_SECRET', 'DISCORD_OAUTH_CLIENT_SECRET', 'DISCORD_CLIENT_SECRET')
    ...
    DASHBOARD_SESSION_SECRET = _env_first('ZENITH_DASHBOARD_SESSION_SECRET', 'ZENITH_DASHBOARD_SESSION_SECRET', 'DASHBOARD_SESSION_SECRET', default=SECRET_KEY)
    DASHBOARD_SESSION_TTL_MS = _env_int('ZENITH_DASHBOARD_SESSION_TTL_MS', _env_int('ZENITH_DASHBOARD_SESSION_TTL_MS', 24 * 60 * 60 * 1000, 3_600_000, 2_592_000_000), 3_600_000, 2_592_000_000)
    DASHBOARD_COOKIE_NAME = _env_first('ZENITH_DASHBOARD_COOKIE_NAME', 'ZENITH_DASHBOARD_COOKIE_NAME', default='zenith_dash')
    MONTHLY_PRICE_DISPLAY = _env_float('MONTHLY_PRICE_DISPLAY', 6.99, 0.01, 10_000)
    PRICE_3MONTH_DISPLAY = _env_float('PRICE_3MONTH_DISPLAY', 12.99, 0.01, 10_000)
    PRO_MONTHLY_PRICE_DISPLAY = _env_float('PRO_MONTHLY_PRICE_DISPLAY', 19.99, 0.01, 10_000)
    LIFETIME_STANDARD_PRICE = _env_int('LIFETIME_STANDARD_PRICE', 25, 1, 25_000)
    LIFETIME_CORE_PRICE_DISPLAY = _env_float('LIFETIME_CORE_PRICE_DISPLAY', 12.99, 0.01, 25_000)
    LIFETIME_PRO_PRICE_DISPLAY = _env_float('LIFETIME_PRO_PRICE_DISPLAY', 34.99, 0.01, 25_000)
    LIFETIME_CORE_CURRENCY = _env_first('LIFETIME_CORE_CURRENCY', default='USD').upper()
    ...
    STRIPE_PRICE_CORE_MONTHLY = _env_first('STRIPE_PRICE_CORE_MONTHLY', 'STRIPE_PRICE_MONTHLY', 'STRIPE_PRICE_BASIC')
    STRIPE_PRICE_PRO_MONTHLY = _env_first('STRIPE_PRICE_PRO_MONTHLY', 'STRIPE_PRICE_3MONTH')
    STRIPE_PRICE_LIFETIME = _env_first('STRIPE_PRICE_LIFETIME', 'STRIPE_PRICE_LIFETIME_PROMO', 'STRIPE_PRICE_PRO')
    STRIPE_PRICE_LIFETIME_CORE = _env_first('STRIPE_PRICE_LIFETIME_CORE')
    STRIPE_PRICE_LIFETIME_PRO = _env_first('STRIPE_PRICE_LIFETIME_PRO')
