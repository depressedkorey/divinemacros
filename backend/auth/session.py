...
from utils.response_signing import sign_auth_response
...
class SessionManager:
    ...
    def create_session(self, license, hwid_fp: str) -> dict:
        ...
        meta = getattr(license, 'extra_metadata', None) or {}
        role = str(meta.get('access_role') or meta.get('role') or 'customer').strip().lower()
        if role not in {'customer', 'moderator', 'admin'}:
            role = 'customer'
        ...
        signing_key = str(current_app.config.get('AUTH_RESPONSE_SIGNING_KEY', '') or '').strip()
        if signing_key:
            response['response_signature'] = sign_auth_response(
                'session',
                [token, license.tier, role, expires_at_iso, hwid_fp],
                signing_key,
            )
        return response
